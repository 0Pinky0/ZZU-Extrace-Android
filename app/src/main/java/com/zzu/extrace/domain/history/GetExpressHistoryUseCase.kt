...
class GetExpressHistoryUseCase {
    operator fun invoke(eid: Int): Response<List<History>> = runBlocking {
        ...
        val response: Response<List<History>> = client.get(GetUrlUseCase("/history/get_by_eid/${eid}")).body()
        client.close()
        response
    }
}